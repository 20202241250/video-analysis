...
def handle_upload():
    ...
    try:
        ...
        # 提取关键帧
        extract_key_frames(filename, file_path, output_folder, 0, None, 10, 15)
...
#     参数：
#     video_path: 输入视频路径
#     output_dir: 输出目录（默认'key_frames'）
#     threshold: 场景检测敏感度（默认30，值越小越敏感）
#     """
...
#         # 遍历所有场景
#         for scene_num, (start_time, end_time) in enumerate(scene_list, 1):
#             # 获取开始帧号和时间戳
...
#                 output_folder,
#                 f"{filename}_scene_{record['scene_num']:03d}_"
#                 f"{record['timestamp']:.2f}s_{record['frame_num']:05d}.jpg"
#             )
    ...
    try:
        ...
        if len(scene_list) == 0:
            ...
            if len(scene_list) > 40:
                ...
                scene_count = 40
                frames_per_scene = 1  # 每个场景只取1帧
                print(f'检测到超过40个场景({len(scene_list)})，已抽样40个代表性场景')
            else:
                ...
                if scene_count == 1:
                    ...
                    frames_per_scene = 1
                print(f'检测到 {scene_count} 个场景')
                print(f'每个场景提取 {frames_per_scene} 帧')
...
def call_behavior_analysis(video_description, user_prompt):
    """调用百炼模型服务-行为分析模型"""
...
def handle_behavior_analysis():
    ...
    try:
        ...
        # 调用行为分析模型
        behavior_analysis = call_behavior_analysis(description, prompt)
...
def call_multimodal_conversation(keyframes):
    ...
    prompt = (
        ...
        # "# 角色\n"
        # "你是一位全球顶尖的幼儿行为观察学者，拥有细致的观察力和敏锐的逻辑分析能力。你擅长用客观、翔实的语言记录每个画面的细节变化。\n\n"
        # "# 核心指令\n"
        # "1. 严格基于输入的关键帧图像序列进行视觉分析，输出《幼儿活动观察报告》。\n"
        # "2. 若检测到非幼儿园场景或画面中无3-6岁幼儿，立即终止并输出：[ERROR] 未检测到幼儿活动。\n\n"
        # "# 思维链（此部分仅作内部推理，不得显式出现在最终答案中）\n"
        # "1. 逐帧仔细观察：先通览所有关键帧，确认是否满足“幼儿园场景+3-6岁幼儿”条件，如不满足则直接报错。\n"
        # "2. 识别每帧中的关键要素：人物特征（性别、外形、服饰）、动作与互动方式，周围环境（可能的幼儿园区域），任何可见文字标识或字幕等。\n"
        # "3. 对每帧信息进行客观、详尽的描述，务必高保真、无遗漏；禁止主观推测、扩大解释。\n"
        # "4. 使用恰当的时间衔接词（如“随后”“同时”“接着”等）将各帧串联，形成一个连贯的故事性描述；保持时序和场景逻辑一致。\n"
        # "5. 明确本段视频可能所在的幼儿园区域（建构区、美工区、益智区、表演区、科学区、图书区、娃娃家、自然角、数学区、沙水区、户外运动区等），但只基于画面真实迹象，不可妄自猜测。\n"
        # "6. 输出应分为3~5个自然段，且全文不少于500个汉字；只作客观叙述，不得包含对幼儿或教师的心理活动描写或评价。\n"
        # "7. 在完成描述后进行终检：如符合要求则正式输出；如不符合则按格式输出错误提示。\n\n"
        # "# 任务描述\n"
        # "给你一个视频片段的多张关键帧图片（以及可能附带的描述信息）。请在以下要求内完成《幼儿观察记录》，除此之外不需要任何多余解释或回复：\n"
        # "1. 逐帧描述：\n"
        # "- 记录所有可见的人物特征、动作、所使用或接触的物品。\n"
        # "- 准确还原画面中的文字或字幕，若有。\n"
        # "- 一句话概括每帧的主要情节，避免遗漏任何显性信息。\n\n"
        # "2. 综合串联：\n"
        # "- 按时间顺序，用自然衔接的语言将各帧形成一条完整叙事。\n"
        # "- 根据场景迹象，判断最可能的幼儿园区域，但不得超出画面本身信息。\n"
        # "- 不要加入任何个人臆测、创意性描述或评价性语言。\n"
        # "- 全文不少于500字，并分3-5个自然段表述。\n\n"
        # "3. 核心限制：\n"
        # "- 不得出现对幼儿内心想法、心理活动或价值评判性的内容。\n"
        # "- 分析范围严格限定在提供的视频画面，禁止延伸至其它背景或常识推测。\n"
        # "- 保持对图像信息的高保真呈现：若无明确画面显示，绝不进行假设。\n\n"
        # "# 输出格式\n"
        # "在满足所有要求后，输出一个不少于500字的文本，包括：\n"
        # "1. 逐帧观察描述（人物、物体、动作、文字、字幕、一句话总结）\n"
        # "2. 时间顺序的综合串联，还原视频片段的剧情与地点推测\n"
        # "3. 以3-5个自然段完成全文叙述\n"
        # "若画面不符合“幼儿园场景+3-6岁幼儿”，则返回：\n"
        # "[ERROR] 未检测到幼儿活动\n\n"
        # "# 强制检测规则\n\n"
        # "- 一旦判断到没有3-6岁幼儿或不在幼儿园环境，立即输出错误提示。\n"
        # "- 禁止在错误提示后提供任何进一步说明。\n\n"
        # "# 错误格式\n"
        # "[ERROR] 未检测到幼儿活动\n\n"
    )
...
# 发送API请求
    response = vl_client.chat.completions.create(
        ...
        messages=messages,
        temperature = 0.01,  # 控制生成多样性，0~1，越低越确定
